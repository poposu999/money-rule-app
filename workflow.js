...
async function v50Locked(fn){if(v50Busy)return;v50Busy=true;try{if(navigator.locks)return await navigator.locks.request('money-rule-prod-v50-write',fn);return await fn();}catch(e){await v50Dialog('操作を完了できませんでした',`<p>${escapeHtml(e.message||'保存前のデータを維持しています。')}</p>`,[['cancel','編集画面に戻る']]);}finally{v50Busy=false;}}
...
function v50BackupObject(){return {format:'money-rule-backup',schemaVersion:50,environment:'production',exportedAt:new Date().toISOString(),state:v50Snapshot(),preferences:{sections:readJson(STORAGE_KEYS.sections,{}),stats:readJson(STORAGE_KEYS.stats,{})}};}
...
function v50DecodeBackup(data){
  if(data?.environment==='development'||data?.format==='money-rule-dev-backup'||Object.keys(data?.localStorage||{}).some(k=>k.startsWith('moneyRuleDev')))throw Error('検証環境のバックアップは本番環境へ復元できません。本番環境のバックアップを選んでください。');
  ...
  if(data?.localStorage){const ls=data.localStorage;const raw=ls.moneyRuleAppV49??ls.moneyRuleAppV2;if(typeof raw!=='string')throw Error('対応する家計簿本体が見つかりません。');const candidate=v50Prepare(JSON.parse(raw));candidate.uiPrefs={};for(const [name,key]of [['sections','moneyRuleSectionPrefs'],['stats','moneyRuleStatPrefs']]){const value=ls[key];if(value!==undefined){const parsed=JSON.parse(value);if(!parsed||typeof parsed!=='object'||Array.isArray(parsed))throw Error('表示設定の形式が不正です。');candidate.uiPrefs[name]=parsed;}}return candidate;}
...
handleBackupObject=async data=>v50Locked(async()=>{
  ...
  if(!state){const raw={};for(const k of ['moneyRuleAppV49','moneyRuleAppV2','moneyRuleV50:head','moneyRuleV50:a','moneyRuleV50:b']){const value=localStorage.getItem(k);if(value!==null)raw[k]=value;}if(Object.keys(raw).length&&!await v50RequireBackup({format:'money-rule-original',localStorage:raw},'復旧前の元データ保存'))return;}
  if(!await v50Dialog('バックアップを復元',`<p>現在の本番データを置き換えます。</p><p>実支出 ${candidate.expenses.length}件／予定支出 ${candidate.plannedExpenses.length}件／固定費 ${candidate.fixedExpenses.length}件</p><p>実支出の期間：${escapeHtml(months[0]||'なし')}〜${escapeHtml(months.at(-1)||'なし')}</p><p>要確認：${issues.length}件。異常なリンクは自動修復しません。</p>`,[['restore','この内容で復元'],['cancel','キャンセル']]))return;
...
exportCurrentBackup=()=>{if(!state)return v50ExportRaw();downloadJson(v50BackupObject(),`money-rule-prod-v50-${getToday()}.json`);setText('backupStatus','バックアップのダウンロードを開始しました。ファイルの保存先をご確認ください。');};
function v50ExportRaw(){const local={};for(const k of ['moneyRuleAppV49','moneyRuleAppV2',STORAGE_KEYS.sections,STORAGE_KEYS.stats]){const v=localStorage.getItem(k);if(v!==null)local[k]=v;}const head=localStorage.getItem('moneyRuleV50:head');if(head!==null){local['moneyRuleV50:head']=head;for(const x of ['a','b'])local['moneyRuleV50:'+x]=localStorage.getItem('moneyRuleV50:'+x);}downloadJson({format:'money-rule-backup',localStorage:local},`money-rule-original-${getToday()}.json`);}
async function v50Migration(rawText,is49){
  const rawBackup={format:'money-rule-backup',schemaVersion:is49?49:48,environment:'production',localStorage:{[is49?'moneyRuleAppV49':'moneyRuleAppV2']:rawText,[STORAGE_KEYS.sections]:localStorage.getItem(STORAGE_KEYS.sections)||'{}',[STORAGE_KEYS.stats]:localStorage.getItem(STORAGE_KEYS.stats)||'{}'}};
  if(!await v50RequireBackup(rawBackup,'新形式への初回移行'))return;
  const raw=JSON.parse(rawText),candidate=v50Prepare(raw);
  candidate.uiPrefs={sections:readJson(STORAGE_KEYS.sections,{}),stats:readJson(STORAGE_KEYS.stats,{})};
...
function v50Boot(){
  ...
  try{
    ...
    const old49=localStorage.getItem('moneyRuleAppV49'),old48=localStorage.getItem('moneyRuleAppV2'),raw=old49??old48;
