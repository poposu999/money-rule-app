...
function calc(){
  ...
  renderCategoryChart(n.es);
  renderDailyChart(n.es,n.variableBudget);
  renderMonthlyChart();
  renderSavingsChart();
}
...
$("expenseList").addEventListener("click",event=>{const edit=event.target.closest("[data-edit]"),del=event.target.closest("[data-delete]");if(edit){event.preventDefault();openEdit(edit.dataset.edit);return}if(del){event.preventDefault();const id=del.dataset.delete,idx=state.expenses.findIndex(x=>String(x.id)===String(id));if(idx>=0){state.expenses.splice(idx,1);save();calc();renderExpenses();}}});
